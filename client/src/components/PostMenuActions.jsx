...
const PostMenuActions = ({post}) => {
  ...
  const {isPending,error,data:savedPosts} = useQuery({
    ...
    const isSaved = savedPosts?.some((item)=>item === post._id) || false
  ...
  return (
    <div>
      ...
      {/* {isPending ? "Loading" : error ? "Failed to fetch daved posts": <div className='flex items-center gap-2 py-2 text-sm cursor-pointer' onClick={handleSave}>
        <svg
